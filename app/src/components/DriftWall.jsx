...
const DriftWall = ({
  ...
  grayscale = false,
  overlayColor = '#060010',
  // Added to the upstream component: tiles live inside a 3D-transformed,
  // masked plane where the browser's lazy-load intersection heuristics never
  // fire, so they can sit unloaded forever. Pass "eager" when the tiles are
  ...
  const renderTile = (item, id, colIndex) => {
    const inner = (
      <span className="drift-wall__inner">
        <img src={item.image} alt={item.title ?? ''} loading={loading} decoding="async" draggable={false} />
