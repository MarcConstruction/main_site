...
const FoldText = ({
  ...
  useEffect(() => {
    ...
    // Resting state = fully unfolded and opaque. Killing a timeline mid-flight
    // otherwise abandons the panels wherever they were, and under React 19
    // StrictMode (mount → cleanup → mount) that left the heading permanently
    // half-invisible. Text must never be able to end up unreadable, so the
    // kill always lands on the readable state; a re-run re-folds from there.
    const settle = () =>
      gsap.set(pieces, {
        opacity: 1,
        rotateX: 0,
    ...
    const killTimeline = ({ restore = true } = {}) => {
    ...
    const play = repeat => {
      // No restore here: fromTo sets the folded start state on the next line.
      killTimeline({ restore: false });
